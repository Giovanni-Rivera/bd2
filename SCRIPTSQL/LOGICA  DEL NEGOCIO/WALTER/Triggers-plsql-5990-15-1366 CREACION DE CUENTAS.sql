...
/*3.1.1 FUNCION QUE NOS DEVUELVE EL NUMERO MAXIMO  ASOCIAONES DE CUENTA, UNA CUENTA PUEDE ESTAR
ASOCIADA COMO MAXIMO CON 3 FIRMAS (CLIENTES)*/
CREATE OR REPLACE FUNCTION MAXIMA_FIRMAS_CUENTA(DPI_CLIENTE VARCHAR2, NUMERO_CUENTA VARCHAR2)
RETURN NUMBER
IS
OBTENER_ID_CLIENTE NUMBER;
RETORNAR_CANTIDAD_FIRMAS NUMBER;
BEGIN
    SELECT FUNCT_OBTENERID_CLIENTE(DPI_CLIENTE) INTO OBTENER_ID_CLIENTE FROM DUAL;

    SELECT COUNT(ID_CLIENTE) INTO RETORNAR_CANTIDAD_FIRMAS FROM CLIENTE_ASOCIADO_CUENTA WHERE
    CLIENTE_ASOCIADO_CUENTA.ID_CUENTA=NUMERO_CUENTA;

    IF(RETORNAR_CANTIDAD_FIRMAS>0 AND RETORNAR_CANTIDAD_FIRMAS<3) THEN
        RETURN 1;
    ELSE
        RETURN 0;
    END IF;
EXCEPTION
    WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('CONTACTE AL DBA FUNCION MAXIMA_CUENTA');
END MAXIMA_FIRMAS_CUENTA;
...
BEGIN
    ...
    IF(EXISTE_CLIENTE=1) THEN
        ...
        IF(TIENE_FIRMA=0) THEN
            ...
            SELECT FUNCT_BUSCAR_AGENCIA INTO NUMERO_AGENCIA FROM DUAL;
...
BEGIN
    /*VAMOS A USAR LA FUNCION PARA CONTAR LA CANTIDAD DE FIRMAS QUE TIENE UNA CUENTA*/
    SELECT MAXIMA_FIRMAS_CUENTA(DPI_CLIENTE,NO_CUENTA) INTO TOTAL_CUENTA_ASOCIADA FROM DUAL;
    ...
    ELSIF(TOTAL_CUENTA_ASOCIADA>3 OR TOTAL_CUENTA_ASOCIADA=0) THEN
...
BEGIN
    SELECT FUNCT_EXISTE_CLIENTE(DPIUNO) INTO EXISTE_CLIENTE_NUEVO FROM DUAL;

    SELECT FUNCT_EXISTE_CLIENTE(DPIDOS) INTO EXISTE_CLIENTE_VIEJO FROM DUAL;
    /*SI AMBOS EXISTEN*/
...
/*VARIABLE PARA DETERMINAR SI EL ID DE CUENTA QUE NOS ESTÁN DANDO YA EXISTE*/
EXISTE_CUENTA NUMBER;
/*RECUPERAR LOS ID DE LOS CLIENTES ASOCIADOS*/
EXTRAER_ID_CLIENTE_NUEVO NUMBER;
/**/
EXTRAER_ID_CLIENTE_VIEJO NUMBER;
/*VARIABLE PARA SABER SI EXISTE LA PATENTE EN LA TABLA EMPRESA*/
EXISTE_PATENTE NUMBER;
/*PARA INDICARLE AL USUARIO QUE NO ESTÁ INGRESANDO UN VALOR CORRECTO O BIEN QUE NO ESTÉ CREADO EL CLIENTE EN  EL SISTEMA*/
DATOS_NO_VALIDOS EXCEPTION;
...
BEGIN
    ...
    IF(EXISTEN_CLIENTES=1) THEN
        ...
        SELECT MAXIMA_FIRMAS_CUENTA(DPI_CLIENTE_NUEVO,NO_CUENTA) INTO TOTAL_CUENTA_ASOCIADA FROM DUAL;

        /*VERIFICO SI LA CUENTA EXISTE, QUIZÁ ES INNECESARIO, PERO VALE LA PENA HACER EL CÁLCULO*/
        SELECT FUNCT_EXISTE_CUENTA(NO_CUENTA) INTO EXISTE_CUENTA FROM DUAL;
        /*EXTRAEMOS EL ID DEL CLIENTE PRA PODERLO ASOCIAR CON LA CUENTA*/
        SELECT FUNCT_OBTENERID_CLIENTE(DPI_CLIENTE_NUEVO) INTO EXTRAER_ID_CLIENTE_NUEVO FROM DUAL;
        /*EXTRAEMOS EL ID DEL CLIENTE PRA PODERLO ASOCIAR CON LA CUENTA*/
        SELECT FUNCT_OBTENERID_CLIENTE(DPI_CLIENTE_VIEJO) INTO EXTRAER_ID_CLIENTE_VIEJO FROM DUAL;
        /*SI EL TOTAL DE CUENTAS ES MAYOR A CERO Y MENOR A TRES, ADEMAS SI LA CUENTA EXISTE Y SI NO MANDAN NUMERO DE PATENTE, PROCEDEMOS A ASOCIAR EL CLIENTE CON LA CUENTA*/
        IF(TOTAL_CUENTA_ASOCIADA>0 AND TOTAL_CUENTA_ASOCIADA<3 AND EXISTE_CUENTA=1 AND NUM_PATENTE IS NULL) THEN
            /*EN ESTE IF VAMOS A METER LA ASOCIACION DE UNA CUENTA COMUN UY CORRIENTE CON UN CLIENTE CON OTRO CLIENTE*/
            /*AL VALIDAR LO ANTERIOR, YA PODEMOS INSERTAR, PERO NECITAMOS EL ID DEL CLIENTE PARA ESO, POR TANTO
            ...
            UPDATE CLIENTE_ASOCIADO_CUENTA SET ID_CLIENTE=EXTRAER_ID_CLIENTE_NUEVO  WHERE ID_CLIENTE=EXTRAER_ID_CLIENTE_VIEJO AND ID_CUENTA=NO_CUENTA;
