...
CUENTA_NUMERO NUMBER;
BEGIN
    SELECT COUNT(ID_CUENTA) INTO CUENTA_NUMERO FROM CUENTA WHERE CUENTA.ID_CUENTA=NUMERO_CUENTA AND CUENTA.ID_ESTATUS=1;
    IF(CUENTA_NUMERO=1) THEN
        RETURN 1;
    ELSE
        RETURN 0;
...
/*funcio para verificar el id de empresa en cliente asociado cuenta y ver que no esté asociada con una empresa*/
create or replace FUNCTION FUNCT_TIENE_EMPRESA(NUMERO_CUENTA VARCHAR2)
RETURN NUMBER
IS
TIENE_EMPRESA NUMBER;
BEGIN
    SELECT COUNT(*) into TIENE_EMPRESA FROM CLIENTE_ASOCIADO_CUENTA WHERE ID_CUENTA=NUMERO_CUENTA AND ID_EMPRESA IS NOT NULL;
    IF(TIENE_EMPRESA=1) THEN
        RETURN 1;
    ELSE
        TIENE_EMPRESA:=0;
        RETURN 0;
    END IF;
END FUNCT_TIENE_EMPRESA;


/*FUNCIÓN PARA EXTRAER EL ID DE LA EMPRESA */
CREATE OR REPLACE FUNCTION FUNCT_EXTRAER_ID_EMPRESA(NUMERO_CUENTA_EMPRESA VARCHAR2) 
RETURN NUMBER
IS
OBTENER_ID_EMPRESA NUMBER;
BEGIN
    SELECT EMP.ID_EMPRESA INTO OBTENER_ID_EMPRESA FROM EMPRESA EMP
    INNER JOIN CLIENTE_ASOCIADO_CUENTA AC
    ON AC.ID_EMPRESA = EMP.ID_EMPRESA
    WHERE ac.id_cuenta=NUMERO_CUENTA_EMPRESA;
    
    IF(OBTENER_ID_EMPRESA=NULL) THEN
        RETURN 0;
        
     ELSE 
        RETURN OBTENER_ID_EMPRESA;
     END IF;   

END FUNCT_EXTRAER_ID_EMPRESA;


SELECT FUNCT_EXTRAER_ID_EMPRESA( '2-2-5-3')FROM DUAL;

COMMIT;



CREATE OR REPLACE PROCEDURE ASOCIAR_CLIENTE_A_EMPRESA(CUENTA_CLIENTE VARCHAR2, CUENTA_EMPRESA VARCHAR2)
IS
/*VARIABLES PARA ALMACENAR EL PROCESO DE EXTRACCIÓN DE VALIDACIÓN DE LAS CUENTAS*/
EXISTE_CUENTA_CLIENTE NUMBER;
EXISTE_CUENTA_EMPRESA NUMBER;
TIENE_EMPRESA NUMBER;
ID_EMPRESA_F NUMBER;
BEGIN 
    /*1.- VALIDAR SI EXISTE LA CUENTA DEL CLIENTE*/
    SELECT FUNCT_EXISTE_CUENTA(CUENTA_CLIENTE) INTO EXISTE_CUENTA_CLIENTE FROM DUAL;
    /*2.- VALIDAR SI EXISTE LA CUENTA DE LA EMPRESA */
    SELECT FUNCT_EXISTE_CUENTA(CUENTA_EMPRESA) INTO EXISTE_CUENTA_EMPRESA FROM DUAL;
    /*3.- antes de operar validamos si la cuenta de empresa existe */
    if(EXISTE_CUENTA_EMPRESA>0) THEN
        /*4.- VALIDAMOS SIA LA CUENTA DEL CLIENTE EXISTE*/
        IF(EXISTE_CUENTA_CLIENTE>0)THEN
            /*5.- VALIDAMOS AHORA SI LA CUENTA DEL CLIENTE NO TIENE ASOCIADA ALGUNA EMPRESA YA*/
                SELECT FUNCT_TIENE_EMPRESA(CUENTA_CLIENTE)INTO TIENE_EMPRESA FROM DUAL;
            /*6.- SI EL RESULTADO ES 0, QUE LE ASOCIE LA CUENTA CON LA EMPRESA*/
            IF(TIENE_EMPRESA=0)THEN
                  /*7.- VAMOS A EXTRAER EL ID DE LA EMPRESA*/  
                 SELECT FUNCT_EXTRAER_ID_EMPRESA(CUENTA_EMPRESA)INTO ID_EMPRESA_F FROM DUAL;
                 /*se valida la extraccion del id*/
                 IF(ID_EMPRESA_F>0) THEN
                    /*SE UPDATEA LA ASOCIACION DE LA CUENTA */
                    UPDATE CLIENTE_ASOCIADO_CUENTA SET ID_EMPRESA=ID_EMPRESA_F WHERE ID_CUENTA=CUENTA_CLIENTE;
                    
                 ELSE
                     RAISE_APPLICATION_ERROR(-20020,'NO EXISTE UNA EMPRESA CON ESE NUMERO DE CUENTA, VERIFIQUE');
                 
                 END IF;
                
            ELSE
                RAISE_APPLICATION_ERROR(-20019,'LA CUENTA DEL CLIENTE YA ESTÁ ASOCIADA A UNA EMPRESA');

            END IF;        
        
        ELSE 
            RAISE_APPLICATION_ERROR(-20018,'EL NUMERO DE CUENTA DEL CLIENTE NO ES VÁLIDO, REVISE POR FAVOR');
        END IF;
    
    ELSE 
        RAISE_APPLICATION_ERROR(-20017,'NO EXISTE UN NUMERO DE CUENTA EMPRESARIAL CON ESE ID, VERIFIQUE POR FAVOR');
    
    END IF;
COMMIT;
EXCEPTION
WHEN INVALID_NUMBER THEN
DBMS_OUTPUT.PUT_LINE('ESTA INGRESANDO UN CARACTER INCORRECTO, VERIFIQUE');
WHEN NO_DATA_FOUND THEN
...
BEGIN
 ASOCIAR_CLIENTE_A_EMPRESA('2-2-3-1','2-2-5-3');
END;
