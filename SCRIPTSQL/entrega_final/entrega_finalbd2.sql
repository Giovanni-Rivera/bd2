/*función para validar si existe la empresa */
create or replace FUNCTION FUNCT_CUENTA_EMPRESA (NUMERO_CUENTA VARCHAR2)
RETURN NUMBER
IS
...
CONTAR_EMPRESA NUMBER;
BEGIN
    SELECT COUNT(C.ID_EMPRESA) INTO CONTAR_EMPRESA FROM CLIENTE_ASOCIADO_CUENTA C 
    INNER JOIN EMPRESA E ON c.id_empresa=E.ID_EMPRESA WHERE C.ID_CUENTA=NUMERO_CUENTA
    AND E.ID_ESTATUS=(SELECT EE.ID_ESTATUS_EMPRESA FROM ESTATUS_EMPRESA EE WHERE EE.NOMBRE='ACTIVA');
    RETURN CONTAR_EMPRESA;
    EXCEPTION
    WHEN NO_DATA_FOUND THEN
            DBMS_OUTPUT.PUT_LINE('LA CUENTA NO ES DE EMPRESA');
    WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE(SQLCODE||'   ERROR :    '||SQLERRM);
END FUNCT_CUENTA_EMPRESA;
...
/*reporte estado general de transaccciones con pago en planilla de la cuenta */
